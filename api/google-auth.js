...
//   ?action=state    (POST, chỉ owner) -> trả state đã ký để mở popup
...
//   ?action=info                      -> thông tin debug (client_id, redirect_uri)
const crypto = require('crypto');
const { configured, checkAuthFull, getConfig, setConfig } = require('./_supa');
...
module.exports = async (req, res) => {
  ...
  if (action === 'info') {
    return res.status(200).json({
      GOOGLE_CLIENT_ID: CID() || '(TRỐNG)',
      has_GOOGLE_CLIENT_SECRET: !!CSECRET(),
      redirect_uri: redirectUri(req),
    });
  }

  // Tạo state — CHỈ owner mới được kết nối studio Drive
  if (action === 'state') {
    const auth = await checkAuthFull(req);
    if (!auth) return res.status(401).json({ error: 'Chưa đăng nhập' });
    if (auth.role !== 'owner') return res.status(403).json({ error: 'Chỉ chủ studio (owner) mới được kết nối Google Drive cho toàn studio' });
  ...
  // Google callback (không có action, hoặc action=callback)
  if (action === 'callback' || (req.query && (req.query.code || req.query.error))) {
    ...
    try {
      ...
      return res.status(200).send(page('Đã kết nối',
        '<h2>✅ Đã kết nối Google Drive của studio</h2>' +
        '<p><b>' + (email || '') + '</b></p>' +
        '<p>Nhân sự giờ có thể tải ảnh lên mà không cần đăng nhập Google.</p>' +
        '<p style="color:#777">Cửa sổ này sẽ tự đóng…</p>' +
